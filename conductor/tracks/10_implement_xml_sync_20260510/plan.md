...
- [x] Task: `quick-xml` を用いた `XmlProjectRepositoryImpl` の実装
...
- [x] Task: `EmEditorGuiDriver` (または相当するインフラ層) における `ExecutePlugin` の呼び出し実装
    - [x] Sub-task: `windows` クレートを用いて `Projects.dll` にメッセージを送信する処理を実装する。
