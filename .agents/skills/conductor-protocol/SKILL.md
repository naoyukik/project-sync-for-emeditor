...
- **徹底した調査**: 新規トラック作成 (`new-track`) および実装 (`implement`) のフェーズでは、必ず `learn-microsoft` ツールを用いた Windows API 調査と、`sdk/` ディレクトリ内の既存実装やドキュメントに基づく EmEditor 仕様調査を行い、その知見を `spec.md` または `plan.md` に反映させよ。
